...
export function listIMessageAccountIds(cfg: KimibotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: KimibotConfig): string {
...
function mergeIMessageAccountConfig(cfg: KimibotConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: KimibotConfig): ResolvedIMessageAccount[] {
