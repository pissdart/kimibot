...
2. **Appends to log file** - Writes a JSON line to `~/.kimibot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.kimibot/logs/commands.log ~/.kimibot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/kimibot`:
   ```
   /home/username/.kimibot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.kimibot/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.kimibot/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.kimibot/logs/commands.log | jq .
```
