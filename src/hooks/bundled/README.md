...
This directory contains hooks that ship with Kimibot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/kimi`)

**Enable**:

```bash
kimibot hooks enable session-memory
```
...
**Output**: `~/.kimibot/logs/commands.log`

**Enable**:

```bash
kimibot hooks enable command-logger
```
...
```bash
kimibot hooks enable soul-evil
```
...
metadata:
  { "kimibot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.kimibot/hooks/` (shared across workspaces)
...
```bash
kimibot hooks info session-memory
```
...
```bash
kimibot hooks enable session-memory
kimibot hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.kimibot/kimibot.json`:
...
2. Restart gateway: `pkill -9 -f 'kimibot.*gateway' && pnpm kimibot gateway`
3. Enable the hook: `kimibot hooks enable my-hook`
