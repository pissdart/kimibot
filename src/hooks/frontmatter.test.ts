...
`;
    ...
    expect(parsed.kimibot.emoji).toBe("💾");
    expect(parsed.kimibot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.kimibot.emoji).toBe("📝");
    expect(parsed.kimibot.events).toEqual(["command"]);
    expect(parsed.kimibot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.kimibot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"kimibot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"kimibot": {"events": ["test"]}}');
  });
...
describe("resolveKimibotMetadata", () => {
  it("extracts kimibot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveKimibotMetadata(frontmatter);
  ...
  it("returns undefined when kimibot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        kimibot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Kimibot" },
            { id: "npm", kind: "npm", package: "@kimibot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@kimibot/hook");
  });
    ...
    "kimibot":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Kimibot" }],
      },
  }
---
...
`;
    ...
    const kimibot = resolveKimibotMetadata(frontmatter);
    expect(kimibot).toBeDefined();
    expect(kimibot?.emoji).toBe("💾");
    expect(kimibot?.events).toEqual(["command:new"]);
    expect(kimibot?.requires?.config).toEqual(["workspace.dir"]);
    expect(kimibot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const kimibot = resolveKimibotMetadata(frontmatter);
    expect(kimibot?.emoji).toBe("disk");
    expect(kimibot?.events).toEqual(["command:new"]);
  });
});
