...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "kimibot-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".kimibot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<KimibotConfig> = {},
): KimibotConfig {
  const base: KimibotConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "kimi"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".kimibot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
