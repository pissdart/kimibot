...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: KimibotConfig;
  nextConfig: KimibotConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.kimibot/kimibot.json (gateway.auth.token) or KIMIBOT_GATEWAY_TOKEN.",
        "Web UI stores a copy in this browser's localStorage (kimibot.control.settings.v1).",
        `Get the tokenized link anytime: ${formatCliCommand("kimibot dashboard --no-open")}`,
      ].join("\n"),
      "Token",
    );
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Kimibot."
            : "Copy/paste this URL in a browser on this machine to control Kimibot.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("kimibot dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          "Kimibot uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("kimibot configure --section web")}`,
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened with your token; keep that tab to control Kimibot."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the tokenized link above."
        : "Onboarding complete. Use the tokenized dashboard link above to control Kimibot.",
  );
}
