...
export function resolveKimiUserDataDir(profileName = DEFAULT_CLAWD_BROWSER_PROFILE_NAME) {
...
export async function launchKimiChrome(
  ...
  if (needsDecorate) {
    try {
      decorateKimiProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 kimi browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`kimi browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`kimi browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 kimi browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopKimiChrome(running: RunningChrome, timeoutMs = 2500) {
