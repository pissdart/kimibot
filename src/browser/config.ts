import type { BrowserConfig, BrowserProfileConfig, KimibotConfig } from "../config/config.js";
import {
...
/**
 * Ensure the default "kimi" profile exists in the profiles map.
 ...
 *
 * Note: this is a Kimibot browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for kimi CDP).
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "kimi";
