...
export function listSignalAccountIds(cfg: KimibotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: KimibotConfig): string {
...
function mergeSignalAccountConfig(cfg: KimibotConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: KimibotConfig): ResolvedSignalAccount[] {
