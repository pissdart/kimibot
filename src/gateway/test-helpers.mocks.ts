...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `kimibot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.KIMIBOT_CONFIG_PATH = path.join(root, "kimibot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "kimibot.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "kimi-gateway-test"),
        ...fileDefaults,
