...
export function resolveHooksConfig(cfg: KimibotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-kimibot-token"] === "string" ? req.headers["x-kimibot-token"].trim() : "";
