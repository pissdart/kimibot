...
import { createKimibotTools } from "../agents/kimibot-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.KIMIBOT_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.KIMIBOT_GATEWAY_PORT = String(gatewayPort);
  process.env.KIMIBOT_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.KIMIBOT_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createKimibotTools().find((candidate) => candidate.name === "sessions_send");
