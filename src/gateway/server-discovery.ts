...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Kimibot";
  if (/kimibot/i.test(trimmed)) return trimmed;
  return `${trimmed} (Kimibot)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.KIMIBOT_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "kimibot");
  ...
  const binCli = path.join(cwd, "bin", "kimibot.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.KIMIBOT_TAILNET_DNS?.trim();
