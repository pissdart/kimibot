...
import { resolveKimibotAgentDir } from "../agents/agent-paths.js";
...
import { ensureKimibotModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { KimibotConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.KIMIBOT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.KIMIBOT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.KIMIBOT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<KimibotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.KIMIBOT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.KIMIBOT_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveKimibotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-live-state-"));
  ...
  process.env.KIMIBOT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.kimibot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveKimibotAgentDir();
  const sanitizedCfg: KimibotConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Kimibot live tool probe (local, safe): " +
    ...
    process.env.KIMIBOT_CONFIG_PATH = previous.configPath;
    process.env.KIMIBOT_GATEWAY_TOKEN = previous.token;
    process.env.KIMIBOT_SKIP_CHANNELS = previous.skipChannels;
    process.env.KIMIBOT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.KIMIBOT_SKIP_CRON = previous.skipCron;
    process.env.KIMIBOT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.KIMIBOT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.KIMIBOT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureKimibotModelsJson(cfg);
      ...
      const rawModels = process.env.KIMIBOT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.kimibot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
