...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.KIMIBOT_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "kimi");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.kimibot-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".kimibot");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "kimibot.json");
      ...
      process.env.KIMIBOT_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.KIMIBOT_CONFIG_PATH = prev.configPath;
        process.env.KIMIBOT_GATEWAY_TOKEN = prev.token;
        process.env.KIMIBOT_SKIP_CHANNELS = prev.skipChannels;
        process.env.KIMIBOT_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.KIMIBOT_SKIP_CRON = prev.skipCron;
        process.env.KIMIBOT_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.KIMIBOT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.KIMIBOT_STATE_DIR = prev.stateDir;
      process.env.KIMIBOT_CONFIG_PATH = prev.configPath;
      process.env.KIMIBOT_GATEWAY_TOKEN = prev.token;
      process.env.KIMIBOT_SKIP_CHANNELS = prev.skipChannels;
      process.env.KIMIBOT_SKIP_GMAIL_WATCHER = prev.skipGmail;
