...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "kimibot doctor",
        managerScriptArgs(manager, "kimibot", ["doctor", "--non-interactive"]),
        gitRoot,
        { KIMIBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
