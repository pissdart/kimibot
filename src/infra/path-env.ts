...
function candidateBinDirs(opts: EnsureKimibotPathOpts): string[] {
  ...
  // Bundled macOS app: `kimibot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingKimibot = path.join(execDir, "kimibot");
    if (isExecutable(siblingKimibot)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/kimibot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "kimibot"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `kimibot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureKimibotCliOnPath(opts: EnsureKimibotPathOpts = {}) {
  if (isTruthyEnvValue(process.env.KIMIBOT_PATH_BOOTSTRAPPED)) return;
  process.env.KIMIBOT_PATH_BOOTSTRAPPED = "1";
