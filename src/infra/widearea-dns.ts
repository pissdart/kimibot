...
export const WIDE_AREA_DISCOVERY_DOMAIN = "kimibot.internal.";
export const WIDE_AREA_ZONE_FILENAME = "kimibot.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*kimibot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "kimibot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "kimibot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "kimibot-gw");
  ...
  records.push(`_kimibot-gw._tcp IN PTR ${instanceLabel}._kimibot-gw._tcp`);
  records.push(`${instanceLabel}._kimibot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._kimibot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; kimibot-content-hash: ${contentHash}\n${contentBody}`;
}
