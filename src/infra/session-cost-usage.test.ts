...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-cost-"));
    ...
    const originalState = process.env.KIMIBOT_STATE_DIR;
    process.env.KIMIBOT_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) delete process.env.KIMIBOT_STATE_DIR;
      else process.env.KIMIBOT_STATE_DIR = originalState;
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-cost-session-"));
