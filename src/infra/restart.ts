...
export function triggerKimibotRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.KIMIBOT_SYSTEMD_UNIT,
        process.env.KIMIBOT_PROFILE,
      );
  ...
  const label =
    process.env.KIMIBOT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.KIMIBOT_PROFILE);
