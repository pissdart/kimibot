...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: KimibotConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: KimibotConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: KimibotConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: KimibotConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: KimibotConfig): void {
