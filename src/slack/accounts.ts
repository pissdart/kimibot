...
export function listSlackAccountIds(cfg: KimibotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: KimibotConfig): string {
...
function mergeSlackAccountConfig(cfg: KimibotConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: KimibotConfig): ResolvedSlackAccount[] {
