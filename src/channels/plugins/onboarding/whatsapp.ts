...
function setWhatsAppDmPolicy(cfg: KimibotConfig, dmPolicy: DmPolicy): KimibotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: KimibotConfig, allowFrom?: string[]): KimibotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: KimibotConfig, selfChatMode: boolean): KimibotConfig {
...
async function detectWhatsAppLinked(cfg: KimibotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Kimibot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Kimibot" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("kimibot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
