...
function setSlackDmPolicy(cfg: KimibotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Kimibot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Kimibot`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/kimi",
          description: "Send a message to Kimibot",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: KimibotConfig, allowFrom: string[]): KimibotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
