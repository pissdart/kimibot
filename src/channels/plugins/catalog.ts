...
import { discoverKimibotPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { KimibotPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  kimibot?: KimibotPackageManifest;
  [LEGACY_MANIFEST_KEY]?: KimibotPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["KIMIBOT_PLUGIN_CATALOG_PATHS", "KIMIBOT_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<KimibotPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: KimibotPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageKimibot?: KimibotPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageKimibot;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.kimibot ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageKimibot: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverKimibotPlugins({ workspaceDir: options.workspaceDir });
