...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.kimibotCanvasA2UIAction ||
        window.webkit.messageHandlers.kimibotCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.kimibotCanvasA2UIAction &&
        typeof window.kimibotCanvasA2UIAction.postMessage === "function") ||
      (window.kimibotCanvasA2UIAction &&
        typeof window.kimibotCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.kimibotSendUserAction === "function" ? window.kimibotSendUserAction : undefined;
  if (!window.kimibotSendUserAction && legacySend) {
    window.kimibotSendUserAction = legacySend;
  }
  if (!window.kimibotSendUserAction && typeof window.kimibotSendUserAction === "function") {
    window.kimibotSendUserAction = window.kimibotSendUserAction;
  }
  const hasHelper = () =>
    typeof window.kimibotSendUserAction === "function" ||
    typeof window.kimibotSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("kimibot:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Kimibot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.kimibotSendUserAction === "function"
        ? window.kimibotSendUserAction
        : window.kimibotSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.KIMIBOT_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "kimi", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Kimibot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
