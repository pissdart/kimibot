...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { kimibot?: { emoji?: string } };
    expect(parsed.kimibot?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { kimibot?: { events?: string[] } };
