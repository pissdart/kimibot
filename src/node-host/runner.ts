...
const execHostEnforced = process.env.KIMIBOT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.KIMIBOT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureKimibotCliOnPath({ pathEnv: process.env.PATH ?? "" });
