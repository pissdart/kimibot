...
import type { KimibotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareKimibotVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "KIMIBOT_GATEWAY_TOKEN",
  "KIMIBOT_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): KimibotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as KimibotConfig;
}
...
function stampConfigVersion(cfg: KimibotConfig): KimibotConfig {
...
function warnIfConfigFromFuture(cfg: KimibotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareKimibotVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Kimibot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: KimibotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): KimibotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as KimibotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as KimibotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: KimibotConfig) {
    clearConfigCache();
...
// module scope. `KIMIBOT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.KIMIBOT_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.KIMIBOT_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): KimibotConfig {
...
export async function writeConfigFile(cfg: KimibotConfig): Promise<void> {
  clearConfigCache();
