...
import type { KimibotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: KimibotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: KimibotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = KimibotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as KimibotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as KimibotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as KimibotConfig)),
    ),
  };
}
