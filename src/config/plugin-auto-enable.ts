import type { KimibotConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: KimibotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: KimibotConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: KimibotConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: KimibotConfig): boolean {
...
function isIMessageConfigured(cfg: KimibotConfig): boolean {
...
function isWhatsAppConfigured(cfg: KimibotConfig): boolean {
...
function isGenericChannelConfigured(cfg: KimibotConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: KimibotConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: KimibotConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: KimibotConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: KimibotConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: KimibotConfig, pluginId: string): KimibotConfig {
...
function enablePluginEntry(cfg: KimibotConfig, pluginId: string): KimibotConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
