...
const ETC_KIMIBOT_DIR = path.join(ROOT_DIR, "etc", "kimibot");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "kimibot.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcKimibotPath(...parts: string[]) {
  return path.join(ETC_KIMIBOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcKimibotPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "kimibot.json"))).toEqual({
      shared: true,
    });
  });
});
