...
/**
 * Cleanup suspended Kimibot CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: KimibotConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-cli-images-"));
  const paths: string[] = [];
