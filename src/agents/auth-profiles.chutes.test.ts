...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.KIMIBOT_AGENT_DIR;
    else process.env.KIMIBOT_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-chutes-"));
    process.env.KIMIBOT_STATE_DIR = tempDir;
    process.env.KIMIBOT_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
