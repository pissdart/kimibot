...
vi.mock("./models-config.js", () => ({
  ensureKimibotModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
}));

vi.mock("./agent-paths.js", () => ({
  resolveKimibotAgentDir: () => "/tmp/kimibot",
}));
...
describe("loadModelCatalog", () => {
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as KimibotConfig });
