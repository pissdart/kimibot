...
const defaultTools = createKimibotCodingTools();

describe("createKimibotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createKimibotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from kimibot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "kimibot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "kimibot-workspace"),
    ...
    const tools = createKimibotCodingTools({ sandbox });
