...
import { createKimibotCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("kimibot-ws-", async (workspaceDir) => {
      await withTempDir("kimibot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createKimibotCodingTools({ workspaceDir });
    ...
    await withTempDir("kimibot-ws-", async (workspaceDir) => {
      await withTempDir("kimibot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello kimibot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("kimibot-ws-", async (workspaceDir) => {
      await withTempDir("kimibot-override-", async (overrideDir) => {
        const tools = createKimibotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("kimibot-sandbox-", async (sandboxDir) => {
      await withTempDir("kimibot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "kimibot-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "kimibot-sandbox:bookworm-slim",
            containerPrefix: "kimibot-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createKimibotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
