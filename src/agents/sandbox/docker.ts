...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "kimibot.sandbox=1");
  args.push("--label", `kimibot.sessionKey=${params.scopeKey}`);
  args.push("--label", `kimibot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `kimibot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "kimibot.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`kimibot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`kimibot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("kimibot sandbox recreate --all");
}
