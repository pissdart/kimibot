...
function resolveExecConfig(cfg: KimibotConfig | undefined) {
...
export function createKimibotCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createKimibotReadTool(freshReadTool)];
    }
