...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-embedded-agent-"));
...
const ensureModels = (cfg: KimibotConfig) => ensureKimibotModelsJson(cfg, agentDir) as unknown;
