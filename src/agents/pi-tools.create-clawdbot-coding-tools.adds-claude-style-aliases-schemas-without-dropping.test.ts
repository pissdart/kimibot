...
import { createKimibotTools } from "./kimibot-tools.js";
import { __testing, createKimibotCodingTools } from "./pi-tools.js";
...
describe("createKimibotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createKimibotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createKimibotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "kimibot-outside.txt");
