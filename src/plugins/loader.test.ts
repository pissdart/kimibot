...
import { loadKimibotPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.KIMIBOT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `kimibot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "kimibot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.KIMIBOT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadKimibotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.KIMIBOT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadKimibotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadKimibotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@kimibot/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        kimibot: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
