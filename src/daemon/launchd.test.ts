...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.KIMIBOT_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.KIMIBOT_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.KIMIBOT_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.KIMIBOT_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.KIMIBOT_TEST_LAUNCHCTL_LOG = logPath;
    process.env.KIMIBOT_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        KIMIBOT_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.KIMIBOT_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.KIMIBOT_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.KIMIBOT_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.KIMIBOT_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when KIMIBOT_PROFILE is default", () => {
  ...
  it("uses default label when KIMIBOT_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when KIMIBOT_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", KIMIBOT_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.molt.jbphoenix.plist",
    );
  });

  it("prefers KIMIBOT_LAUNCHD_LABEL over KIMIBOT_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      KIMIBOT_PROFILE: "jbphoenix",
      KIMIBOT_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from KIMIBOT_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      KIMIBOT_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty KIMIBOT_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      KIMIBOT_PROFILE: "myprofile",
      KIMIBOT_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from KIMIBOT_PROFILE", () => {
    const env = { HOME: "/Users/test", KIMIBOT_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.molt.myprofile.plist",
    );
  });
