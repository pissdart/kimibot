...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.KIMIBOT_GATEWAY_PORT).toBe("18789");
    expect(env.KIMIBOT_GATEWAY_TOKEN).toBe("secret");
    expect(env.KIMIBOT_SERVICE_MARKER).toBe("kimibot");
    expect(env.KIMIBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.KIMIBOT_SERVICE_VERSION).toBe("string");
    expect(env.KIMIBOT_SYSTEMD_UNIT).toBe("kimibot-gateway.service");
    if (process.platform === "darwin") {
      expect(env.KIMIBOT_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", KIMIBOT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.KIMIBOT_SYSTEMD_UNIT).toBe("kimibot-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.KIMIBOT_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
