...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.KIMIBOT_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.KIMIBOT_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.KIMIBOT_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.KIMIBOT_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
