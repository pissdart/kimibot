...
beforeAll(() => {
  previousProfile = process.env.KIMIBOT_PROFILE;
  process.env.KIMIBOT_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.KIMIBOT_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/kimibot-root.js", () => ({
  resolveKimibotPackageRoot: vi.fn().mockResolvedValue("/tmp/kimibot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/kimibot/pnpm-lock.yaml",
      markerPath: "/tmp/kimibot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Kimibot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("kimibot status --all") ||
          l.includes("kimibot --profile isolated status --all") ||
          l.includes("kimibot status --all") ||
          l.includes("kimibot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.KIMIBOT_GATEWAY_TOKEN = "abcd1234";
    try {
