...
function resolveConfiguredModelRaw(cfg: KimibotConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: KimibotConfig): string[] {
...
export function applyPrimaryModel(cfg: KimibotConfig, model: string): KimibotConfig {
...
export function applyModelAllowlist(cfg: KimibotConfig, models: string[]): KimibotConfig {
  const defaults = cfg.agents?.defaults;
