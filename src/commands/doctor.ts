...
function resolveMode(cfg: KimibotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Kimibot doctor");

  const root = await resolveKimibotPackageRoot({
  ...
  let cfg: KimibotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("kimibot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("kimibot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("kimibot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("kimibot doctor --fix")}" to apply changes.`);
  }
