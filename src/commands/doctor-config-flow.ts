...
function stripUnknownConfigKeys(config: KimibotConfig): {
  config: KimibotConfig;
  removed: string[];
} {
  const parsed = KimibotSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as KimibotConfig;
...
function noteOpencodeProviderOverrides(cfg: KimibotConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.KIMIBOT_CONFIG_PATH?.trim() || env.KIMIBOT_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "kimibot.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "kimibot.json");
  ...
  let cfg: KimibotConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as KimibotConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("kimibot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("kimibot doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "kimibot doctor --fix" to remove these keys.');
    }
  }
