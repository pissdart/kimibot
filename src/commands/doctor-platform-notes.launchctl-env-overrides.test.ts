...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "KIMIBOT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("KIMIBOT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv KIMIBOT_GATEWAY_TOKEN");
    expect(message).not.toContain("KIMIBOT_GATEWAY_PASSWORD");
  });
