...
const requireAgentDir = () => {
  const agentDir = process.env.KIMIBOT_AGENT_DIR;
  if (!agentDir) throw new Error("KIMIBOT_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.KIMIBOT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under KIMIBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-oauth-"));
    process.env.KIMIBOT_STATE_DIR = tempStateDir;
    process.env.KIMIBOT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.KIMIBOT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to KIMIBOT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-minimax-"));
    process.env.KIMIBOT_STATE_DIR = tempStateDir;
    process.env.KIMIBOT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.KIMIBOT_AGENT_DIR;
