...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".kimibot", "disable-launchagent");
...
function hasConfigGatewayCreds(cfg: KimibotConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("KIMIBOT_GATEWAY_TOKEN");
  const envPassword = await getenv("KIMIBOT_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `KIMIBOT_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `KIMIBOT_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv KIMIBOT_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv KIMIBOT_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
