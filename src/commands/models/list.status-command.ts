...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("kimibot models auth setup-token")}\` or \`${formatCliCommand("kimibot configure")}\`.`
          : `Run \`${formatCliCommand("kimibot configure")}\` or set an API key env var.`;
