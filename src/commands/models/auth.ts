...
import { readConfigFileSnapshot, type KimibotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: KimibotConfig, model: string): KimibotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("kimibot plugins install")}\`.`,
    );
  }
