...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveKimibotAgentDir: vi.fn().mockReturnValue("/tmp/kimibot-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveKimibotAgentDir: mocks.resolveKimibotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/kimibot-agent/auth-profiles.json");
