...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "kimibot-doctor-"));
  tempRoot = root;
  return root;
}
...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    const detected = await detectLegacyStateMigrations({
      cfg,
      env: { KIMIBOT_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    const cfg: KimibotConfig = { session: { mainKey: "work" } };
    const legacySessionsDir = path.join(root, "sessions");
    fs.mkdirSync(legacySessionsDir, { recursive: true });
    writeJson5(path.join(legacySessionsDir, "sessions.json"), {
      "+1555": { sessionId: "a", updatedAt: 10 },
  ...
  it("auto-migrates legacy state dir to ~/.kimibot", async () => {
    const root = await makeTempRoot();
    ...
    const targetDir = path.join(root, ".kimibot");
  ...
  it("skips state dir migration when env override is set", async () => {
    ...
    const result = await autoMigrateLegacyStateDir({
      env: { KIMIBOT_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      homedir: () => root,
    });
