...
beforeEach(() => {
  ...
  resolveKimibotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.KIMIBOT_STATE_DIR;
  originalUpdateInProgress = process.env.KIMIBOT_UPDATE_IN_PROGRESS;
  process.env.KIMIBOT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "kimibot-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.KIMIBOT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.KIMIBOT_UPDATE_IN_PROGRESS;
  } else {
    process.env.KIMIBOT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveKimibotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadKimibotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/kimibot-root.js", () => ({
  resolveKimibotPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "kimibot-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.KIMIBOT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when KIMIBOT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/kimibot.json",
      exists: true,
      raw: "{}",
    ...
    process.env.KIMIBOT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
