...
export function listAgentEntries(cfg: KimibotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: KimibotConfig, agentId: string) {
...
function resolveAgentModel(cfg: KimibotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: KimibotConfig): AgentSummary[] {
