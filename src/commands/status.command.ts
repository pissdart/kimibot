...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Kimibot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("kimibot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("kimibot security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("kimibot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("kimibot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("kimibot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("kimibot gateway probe")}`);
  }
}
