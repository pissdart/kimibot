...
import type { KimibotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: KimibotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("kimibot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("kimibot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("kimibot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("kimibot doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "kimibot config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("kimibot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
