...
function resolveDefaultAccountId(cfg: KimibotConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: KimibotConfig): boolean {
...
export function summarizeBindings(cfg: KimibotConfig, bindings: AgentBinding[]): string[] {
