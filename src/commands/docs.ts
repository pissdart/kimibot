...
const SEARCH_TOOL = "https://docs.molt.bot/mcp.SearchKimibot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('kimibot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.molt.bot/");
      runtime.log(`Search: ${formatCliCommand('kimibot docs "your query"')}`);
    }
    return;
  }
