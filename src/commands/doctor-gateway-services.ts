...
export async function maybeMigrateLegacyGatewayService(
  ...
  const migrate = await prompter.confirmSkipInNonInteractive({
    message: "Migrate legacy gateway services to Kimibot now?",
    initialValue: true,
  });
  if (!migrate) return;
  ...
  if (loaded) {
    note(`Kimibot ${service.label} already ${service.loadedText}.`, "Gateway");
    return;
  }

  const install = await prompter.confirmSkipInNonInteractive({
    message: "Install Kimibot gateway service now?",
    initialValue: true,
  });
  if (!install) return;
  ...
  const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
    env: process.env,
    port,
    token: cfg.gateway?.auth?.token ?? process.env.KIMIBOT_GATEWAY_TOKEN,
