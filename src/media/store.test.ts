...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "KIMIBOT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.KIMIBOT_STATE_DIR = path.join(home, ".kimibot");
    ...
    await fs.mkdir(path.join(home, ".kimibot"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.kimibot${path.sep}media`);
