...
import { scheduleGatewaySigusr1Restart, triggerKimibotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting kimibot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerKimibotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting kimibot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
