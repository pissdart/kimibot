...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "kimibot-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      KIMIBOT_NO_RESPAWN: "1",
      KIMIBOT_STATE_DIR: stateDir,
      KIMIBOT_SKIP_CHANNELS: "1",
      KIMIBOT_SKIP_GMAIL_WATCHER: "1",
      KIMIBOT_SKIP_CRON: "1",
      KIMIBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
      KIMIBOT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "kimibot-entry-bootstrap.mjs");
