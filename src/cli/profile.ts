...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.kimibot${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.KIMIBOT_PROFILE = profile;

  const stateDir = env.KIMIBOT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.KIMIBOT_STATE_DIR?.trim()) env.KIMIBOT_STATE_DIR = stateDir;

  if (!env.KIMIBOT_CONFIG_PATH?.trim()) {
    env.KIMIBOT_CONFIG_PATH = path.join(stateDir, "kimibot.json");
  }

  if (profile === "dev" && !env.KIMIBOT_GATEWAY_PORT?.trim()) {
    env.KIMIBOT_GATEWAY_PORT = "19001";
  }
}
