...
function appendKimiHubHint(output: string, json?: boolean): string {
  if (json) return output;
  return `${output}\n\nTip: use \`npx kimihub\` to search, install, and sync skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("kimibot skills list")}\` to see all skills.`
      : "No skills found.";
    return appendKimiHubHint(message, opts.json);
  }
  ...
  return appendKimiHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendKimiHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("kimibot skills list")}\` to see available skills.`,
      opts.json,
    );
  }
