...
import { resolveKimibotPackageRoot } from "../infra/kimibot-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "kimibot doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "kimibot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "kimibot"]);
const CLI_NAME = resolveCliName();
const KIMIBOT_REPO_URL = "https://github.com/kimibot/kimibot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "kimibot");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("kimibot@")) return trimmed.slice("kimibot@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.KIMIBOT_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", KIMIBOT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `KIMIBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set KIMIBOT_GIT_DIR to an empty folder or a kimibot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`KIMIBOT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveKimibotPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Kimibot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Kimibot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Kimibot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("kimibot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("kimibot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g kimibot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g kimibot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.KIMIBOT_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.KIMIBOT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("kimibot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("kimibot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("kimibot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("kimibot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `kimibot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `KIMIBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set KIMIBOT_GIT_DIR to an empty folder or a kimibot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via KIMIBOT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Kimibot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["kimibot update", "Update a source checkout (git)"],
        ["kimibot update --channel beta", "Switch to beta channel (git + npm)"],
        ["kimibot update --channel dev", "Switch to dev channel (git + npm)"],
        ["kimibot update --tag beta", "One-off update to a dist-tag or version"],
        ["kimibot update --no-restart", "Update without restarting the service"],
        ["kimibot update --json", "Output result as JSON"],
        ["kimibot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["kimibot update wizard", "Interactive update wizard"],
        ["kimibot --update", "Shorthand for kimibot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run kimibot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["kimibot update status", "Show channel + version status."],
          ["kimibot update status --json", "JSON output."],
          ["kimibot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
