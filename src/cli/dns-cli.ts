...
export function registerDnsCli(program: Command) {
  ...
  dns
    .command("setup")
    .description("Set up CoreDNS to serve kimibot.internal for unicast DNS-SD (Wide-Area Bonjour)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.kimibot/kimibot.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): kimibot.internal"));
      ...
      const serverPath = path.join(confDir, "kimibot.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by kimibot dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.kimibot/kimibot.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
