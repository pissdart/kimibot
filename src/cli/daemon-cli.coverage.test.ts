...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    KIMIBOT_STATE_DIR: process.env.KIMIBOT_STATE_DIR,
    KIMIBOT_CONFIG_PATH: process.env.KIMIBOT_CONFIG_PATH,
    KIMIBOT_GATEWAY_PORT: process.env.KIMIBOT_GATEWAY_PORT,
    KIMIBOT_PROFILE: process.env.KIMIBOT_PROFILE,
  };

  beforeEach(() => {
    process.env.KIMIBOT_STATE_DIR = "/tmp/kimibot-cli-state";
    process.env.KIMIBOT_CONFIG_PATH = "/tmp/kimibot-cli-state/kimibot.json";
    delete process.env.KIMIBOT_GATEWAY_PORT;
    delete process.env.KIMIBOT_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.KIMIBOT_STATE_DIR !== undefined)
      process.env.KIMIBOT_STATE_DIR = originalEnv.KIMIBOT_STATE_DIR;
    else delete process.env.KIMIBOT_STATE_DIR;

    if (originalEnv.KIMIBOT_CONFIG_PATH !== undefined)
      process.env.KIMIBOT_CONFIG_PATH = originalEnv.KIMIBOT_CONFIG_PATH;
    else delete process.env.KIMIBOT_CONFIG_PATH;

    if (originalEnv.KIMIBOT_GATEWAY_PORT !== undefined)
      process.env.KIMIBOT_GATEWAY_PORT = originalEnv.KIMIBOT_GATEWAY_PORT;
    else delete process.env.KIMIBOT_GATEWAY_PORT;

    if (originalEnv.KIMIBOT_PROFILE !== undefined)
      process.env.KIMIBOT_PROFILE = originalEnv.KIMIBOT_PROFILE;
    else delete process.env.KIMIBOT_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        KIMIBOT_PROFILE: "dev",
        KIMIBOT_STATE_DIR: "/tmp/kimibot-daemon-state",
        KIMIBOT_CONFIG_PATH: "/tmp/kimibot-daemon-state/kimibot.json",
        KIMIBOT_GATEWAY_PORT: "19001",
      },
