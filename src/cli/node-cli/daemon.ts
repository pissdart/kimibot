...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("kimibot node install"), formatCliCommand("kimibot node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("kimibot node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    KIMIBOT_LOG_PREFIX: baseEnv.KIMIBOT_LOG_PREFIX ?? "node",
