...
This document describes how the Kimibot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`kimibot acp` exposes an ACP agent over stdio and forwards prompts to a running
Kimibot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Kimibot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `kimibot acp` over stdio.

Example config:

```bash
kimibot config set gateway.remote.url wss://gateway-host:18789
kimibot config set gateway.remote.token <token>
```

Example run:

```bash
kimibot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
kimibot acp --session agent:main:main
kimibot acp --session agent:design:main
kimibot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Kimibot ACP” to start a thread.

## Execution Model

- ACP client spawns `kimibot acp` and speaks ACP messages over stdio.
...
```bash
kimibot acp --session agent:main:main
kimibot acp --session-label "support inbox"
kimibot acp --reset-session
```
...
`kimibot acp` resolves the Gateway URL and auth from CLI flags or config:
