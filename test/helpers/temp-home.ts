...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("KIMIBOT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.KIMIBOT_STATE_DIR = path.join(base, ".kimibot");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "kimibot-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".kimibot", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
