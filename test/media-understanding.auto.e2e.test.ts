...
import type { KimibotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("kimibot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("kimibot-bin-sherpa-");
      const modelDir = await makeTempDir("kimibot-sherpa-model-");
      ...
      const cfg: KimibotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("kimibot-bin-whispercpp-");
      const modelDir = await makeTempDir("kimibot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("kimibot-bin-gemini-");
      ...
      const cfg: KimibotConfig = { tools: { media: { image: {} } } };
