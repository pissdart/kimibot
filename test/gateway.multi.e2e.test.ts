...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `kimibot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".kimibot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          KIMIBOT_CONFIG_PATH: configPath,
          KIMIBOT_STATE_DIR: stateDir,
          KIMIBOT_GATEWAY_TOKEN: "",
          KIMIBOT_GATEWAY_PASSWORD: "",
          KIMIBOT_SKIP_CHANNELS: "1",
          KIMIBOT_SKIP_BROWSER_CONTROL_SERVER: "1",
          KIMIBOT_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        KIMIBOT_GATEWAY_TOKEN: inst.gatewayToken,
        KIMIBOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          KIMIBOT_GATEWAY_PORT: String(gwA.port),
          KIMIBOT_GATEWAY_TOKEN: gwA.gatewayToken,
          KIMIBOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          KIMIBOT_GATEWAY_PORT: String(gwB.port),
          KIMIBOT_GATEWAY_TOKEN: gwB.gatewayToken,
          KIMIBOT_GATEWAY_PASSWORD: "",
        }),
