...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "KimibotChatUI")

@MainActor
@Observable
public final class KimibotChatViewModel {
    public private(set) var messages: [KimibotChatMessage] = []
    ...
    public var attachments: [KimibotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [KimibotChatPendingToolCall] = []
    public private(set) var sessions: [KimibotChatSessionEntry] = []
    private let transport: any KimibotChatTransport
    ...
    private var pendingToolCallsById: [String: KimibotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any KimibotChatTransport) {
    ...
    public var sessionChoices: [KimibotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [KimibotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [KimibotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: KimibotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [KimibotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: KimibotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [KimibotChatMessage]) -> [KimibotChatMessage] {
        var result: [KimibotChatMessage] = []
    ...
    private static func dedupeKey(for message: KimibotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [KimibotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> KimibotChatAttachmentPayload in
            KimibotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> KimibotChatSessionEntry {
        KimibotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: KimibotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: KimibotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: KimibotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = KimibotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> KimibotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
