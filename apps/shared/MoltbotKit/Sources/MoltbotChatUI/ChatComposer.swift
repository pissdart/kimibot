...
@MainActor
struct KimibotChatComposer: View {
    @Bindable var viewModel: KimibotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(KimibotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(KimibotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \KimibotPendingAttachment.id)
                { (att: KimibotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            KimibotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(KimibotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(KimibotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(KimibotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
