...
private final class TestChatTransport: @unchecked Sendable, KimibotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [KimibotChatHistoryPayload]
    private let sessionsResponses: [KimibotChatSessionsListResponse]

    private let stream: AsyncStream<KimibotChatTransportEvent>
    private let continuation: AsyncStream<KimibotChatTransportEvent>.Continuation

    init(
        historyResponses: [KimibotChatHistoryPayload],
        sessionsResponses: [KimibotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<KimibotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> KimibotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? KimibotChatHistoryPayload(
        ...
        attachments _: [KimibotChatAttachmentPayload]) async throws -> KimibotChatSendResponse
    {
        ...
        return KimibotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> KimibotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? KimibotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: KimibotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = KimibotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = KimibotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { KimibotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = KimibotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = KimibotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = KimibotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { KimibotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
