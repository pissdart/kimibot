## Kimibot Node (Android) (internal)

Modern Android node app: connects to the **Gateway WebSocket** (`_kimibot-gw._tcp`) and exposes **Canvas + Chat + Camera**.
...
```bash
pnpm kimibot gateway --port 18789 --verbose
```
...
```bash
kimibot nodes pending
kimibot nodes approve <requestId>
```
