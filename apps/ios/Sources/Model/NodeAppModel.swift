...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = KimibotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = KimibotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = KimibotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = KimibotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = KimibotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__kimibot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: KimibotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for kimibot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case KimibotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case KimibotCanvasCommand.present.rawValue,
                 KimibotCanvasCommand.hide.rawValue,
                 KimibotCanvasCommand.navigate.rawValue,
                 KimibotCanvasCommand.evalJS.rawValue,
                 KimibotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case KimibotCanvasA2UICommand.reset.rawValue,
                 KimibotCanvasA2UICommand.push.rawValue,
                 KimibotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case KimibotCameraCommand.list.rawValue,
                 KimibotCameraCommand.snap.rawValue,
                 KimibotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case KimibotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: KimibotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: KimibotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(KimibotLocationGetParams.self, from: req.paramsJSON)) ??
            KimibotLocationGetParams()
        ...
        let payload = KimibotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case KimibotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(KimibotCanvasPresentParams.self, from: req.paramsJSON)) ??
                KimibotCanvasPresentParams()
        ...
        case KimibotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case KimibotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(KimibotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case KimibotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(KimibotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case KimibotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(KimibotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case KimibotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.kimibotA2UI) return JSON.stringify({ ok: false, error: "missing kimibotA2UI" });
              return JSON.stringify(globalThis.kimibotA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case KimibotCanvasA2UICommand.push.rawValue, KimibotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == KimibotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(KimibotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try KimibotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(KimibotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try KimibotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.kimibotA2UI) return JSON.stringify({ ok: false, error: "missing kimibotA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.kimibotA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case KimibotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case KimibotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(KimibotCameraSnapParams.self, from: req.paramsJSON)) ??
                KimibotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case KimibotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(KimibotCameraClipParams.self, from: req.paramsJSON)) ??
                KimibotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(KimibotScreenRecordParams.self, from: req.paramsJSON)) ??
            KimibotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> KimibotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return KimibotLocationMode(rawValue: raw) ?? .off
    }
