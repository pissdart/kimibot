...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [KimibotCapability.canvas.rawValue, KimibotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(KimibotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(KimibotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = KimibotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(KimibotCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            KimibotSystemCommand.notify.rawValue,
            KimibotSystemCommand.which.rawValue,
            KimibotSystemCommand.run.rawValue,
            KimibotSystemCommand.execApprovalsGet.rawValue,
            KimibotSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(KimibotCapability.camera.rawValue) {
            commands.append(KimibotCameraCommand.list.rawValue)
            commands.append(KimibotCameraCommand.snap.rawValue)
            commands.append(KimibotCameraCommand.clip.rawValue)
        }
        if caps.contains(KimibotCapability.location.rawValue) {
            commands.append(KimibotLocationCommand.get.rawValue)
        }
