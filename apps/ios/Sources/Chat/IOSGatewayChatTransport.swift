...
struct IOSGatewayChatTransport: KimibotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> KimibotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(KimibotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> KimibotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(KimibotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [KimibotChatAttachmentPayload]) async throws -> KimibotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [KimibotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(KimibotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(KimibotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KimibotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KimibotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KimibotAgentEventPayload.self)
                        {
