...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: KimibotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: KimibotProtocol.AnyCodable] = [
            "presence": .init([KimibotProtocol.AnyCodable(entry)]),
        ]
        let payload = KimibotProtocol.AnyCodable(payloadMap)
