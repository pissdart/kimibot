...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/Kimibot/canvas/<session>/`).
...
public let controlSocketPath = FileManager()
    ...
    .appendingPathComponent("Library/Application Support/kimibot/control.sock")
    .path
