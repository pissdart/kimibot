...
struct ConnectOptions {
    ...
    var clientId: String = "kimibot-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: kimibot-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Kimibot macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Kimibot macOS Gateway Connect")
...
private func resolvedToken(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["KIMIBOT_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
    if mode == "remote" {
...
private func resolvedPassword(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["KIMIBOT_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if mode == "remote" {
