...
enum DebugActions {
    private static let verboseDefaultsKey = "kimibot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "kimibot.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Kimibot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".kimibot/kimibot.json")
        guard
