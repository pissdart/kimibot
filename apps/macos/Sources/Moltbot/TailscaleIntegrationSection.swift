...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.kimibot/kimibot.json. Prefer KIMIBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.kimibot/kimibot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.kimibot/kimibot.json. Restart the gateway to apply."
        }
