...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["KIMIBOT_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/kimibot")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("kimibot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("kimibot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/kimibot/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("kimibot") && $0.pathExtension == "log" }
            .max { lhs, rhs in
