...
enum CommandResolver {
    private static let projectRootDefaultsKey = "kimibot.gatewayProjectRootPath"
    private static let helperName = "kimibot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/kimibot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/kimibot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let kimibotPaths = self.kimibotManagedPaths(home: home)
        if !kimibotPaths.isEmpty {
            extras.insert(contentsOf: kimibotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + kimibotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func kimibotManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".kimibot")
    ...
    static func kimibotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectKimibotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/kimibot.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyKimibotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.kimibotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func kimibotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let kimibotPath = self.projectKimibotExecutable(projectRoot: root) {
                return [kimibotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "kimibot", subcommand] + extraArgs
            }
            if let kimibotPath = self.kimibotExecutable(searchPaths: searchPaths) {
                return [kimibotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            kimibot entrypoint missing (looked for dist/index.js or bin/kimibot.js); run pnpm build.
            """
    ...
    // Existing callers still refer to kimibotCommand; keep it as node alias.
    static func kimibotCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real kimibot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/kimibot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v kimibot >/dev/null 2>&1; then
          CLI="$(command -v kimibot)"
          kimibot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/kimibot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/kimibot.js"
            node "$PRJ/bin/kimibot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent kimibot"
          pnpm --silent kimibot \(quotedArgs);
        else
          echo "kimibot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? KimibotConfigFile.loadDict()
