...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: KimibotOAuthStore.AnthropicOAuthStatus = KimibotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: KimibotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = KimibotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = KimibotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try KimibotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Kimibot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: KimibotOAuthStore.AnthropicOAuthStatus,
