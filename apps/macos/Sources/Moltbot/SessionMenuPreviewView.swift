...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> KimibotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: KimibotSessionPreviewEntry,
    ...
    private static func cache(payload: KimibotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: KimibotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [KimibotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [KimibotKit.AnyCodable]) -> [KimibotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(KimibotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: KimibotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: KimibotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: KimibotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: KimibotChatMessage) -> String? {
