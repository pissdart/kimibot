...
actor PortGuardian {
    ...
    private nonisolated static let appSupportDir: URL = {
        ...
        return base.appendingPathComponent("Kimibot", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "kimibot", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `kimibot` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a kimibot listener as expected.
            if cmd.contains("kimibot"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
