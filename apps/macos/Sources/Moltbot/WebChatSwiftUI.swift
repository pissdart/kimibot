...
struct MacGatewayChatTransport: KimibotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> KimibotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(KimibotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> KimibotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                KimibotGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<KimibotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any KimibotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = KimibotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: KimibotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<KimibotChatView>) -> NSViewController
    {
