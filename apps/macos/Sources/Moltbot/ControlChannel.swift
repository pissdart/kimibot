...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: KimibotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: KimibotKit.AnyCodable]()) {
                $0[$1.key] = KimibotKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or KIMIBOT_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: KimibotProtocol.AnyCodable?) -> [String: KimibotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: KimibotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: KimibotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: KimibotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("kimibot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("kimibot.control.agent")
}
