...
enum KimibotLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = KimibotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = KimibotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        KimibotLogging.bootstrapIfNeeded()
        let label = KimibotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct KimibotOSLogHandler: LogHandler {
...
struct KimibotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = KimibotLogging.parseLabel(self.label)
