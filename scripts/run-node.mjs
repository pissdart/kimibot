...
const compiler = env.KIMIBOT_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.KIMIBOT_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.KIMIBOT_RUNNER_LOG === "0") return;
  process.stderr.write(`[kimibot] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["kimibot.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
