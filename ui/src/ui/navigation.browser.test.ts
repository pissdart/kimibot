...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/kimibot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/kimibot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/kimibot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__KIMIBOT_CONTROL_UI_BASE_PATH__ = "/kimibot";
    const app = mountApp("/kimibot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/kimibot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/kimibot/sessions");
  });
