...
import type { KimibotApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as KimibotApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as KimibotApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as KimibotApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as KimibotApp);
    await loadDevices(host as unknown as KimibotApp);
    await loadConfig(host as unknown as KimibotApp);
    await loadExecApprovals(host as unknown as KimibotApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as KimibotApp);
    await loadConfig(host as unknown as KimibotApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as KimibotApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as KimibotApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__KIMIBOT_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as KimibotApp, false),
    loadPresence(host as unknown as KimibotApp),
    loadSessions(host as unknown as KimibotApp),
    loadCronStatus(host as unknown as KimibotApp),
    loadDebug(host as unknown as KimibotApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as KimibotApp, true),
    loadConfigSchema(host as unknown as KimibotApp),
    loadConfig(host as unknown as KimibotApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as KimibotApp, false),
    loadCronStatus(host as unknown as KimibotApp),
    loadCronJobs(host as unknown as KimibotApp),
  ]);
}
