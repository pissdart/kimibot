...
import type { PluginRuntime } from "kimibot/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.KIMIBOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-nostr-"));
  process.env.KIMIBOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.KIMIBOT_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".kimibot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.KIMIBOT_STATE_DIR;
    else process.env.KIMIBOT_STATE_DIR = previous;
