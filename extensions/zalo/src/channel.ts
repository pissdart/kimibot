...
export const zaloDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZaloAccount({ cfg: cfg as KimibotConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZaloAccountIds(cfg as KimibotConfig),
    resolveAccount: (cfg, accountId) => resolveZaloAccount({ cfg: cfg as KimibotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZaloAccountId(cfg as KimibotConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as KimibotConfig).channels?.zalo?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const account = resolveZaloAccount({ cfg: cfg as KimibotConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZaloAccount({ cfg: cfg as KimibotConfig });
