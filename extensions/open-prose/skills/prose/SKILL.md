...
metadata: {"kimibot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
## Kimibot Runtime Mapping

- **Task tool** in the upstream spec == Kimibot `sessions_spawn`
- **File I/O** == Kimibot `read`/`write`
- **Remote fetch** == Kimibot `web_fetch` (or `exec` with curl when POST is required)
