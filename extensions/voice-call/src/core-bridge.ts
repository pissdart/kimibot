...
function resolveKimibotRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.KIMIBOT_ROOT?.trim() || process.env.KIMIBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["kimibot", "kimibot"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set KIMIBOT_ROOT (or legacy KIMIBOT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveKimibotRoot();
