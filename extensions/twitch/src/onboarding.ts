...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: KIMIBOT_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: KimibotConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var KIMIBOT_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as KimibotConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as KimibotConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.KIMIBOT_TWITCH_ACCESS_TOKEN?.trim();
