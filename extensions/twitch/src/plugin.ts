/**
 * Twitch channel plugin for Kimibot.
 *
...
import { buildChannelConfigSchema } from "kimibot/plugin-sdk";
 ...
 * for Kimibot. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: KimibotConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: KimibotConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: KimibotConfig): boolean => {
