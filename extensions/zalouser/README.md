...
Kimibot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
kimibot channels login --channel zalouser
...
```bash
kimibot message send --channel zalouser --target <threadId> --message "Hello from Kimibot!"
```
...
```bash
kimibot channels login --channel zalouser              # Login via QR
kimibot channels login --channel zalouser --account work
kimibot channels status --probe
kimibot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
kimibot directory self --channel zalouser
kimibot directory peers list --channel zalouser --query "name"
kimibot directory groups list --channel zalouser --query "work"
kimibot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
kimibot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
kimibot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `kimibot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
kimibot channels login --channel zalouser --account work
kimibot message send --channel zalouser --account work --target <id> --message "Hello"
