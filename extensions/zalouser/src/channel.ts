...
function resolveZalouserGroupToolPolicy(
  ...
  const account = resolveZalouserAccountSync({
    cfg: params.cfg as KimibotConfig,
...
export const zalouserDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZalouserAccountSync({ cfg: cfg as KimibotConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZalouserAccountIds(cfg as KimibotConfig),
    resolveAccount: (cfg, accountId) =>
      resolveZalouserAccountSync({ cfg: cfg as KimibotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZalouserAccountId(cfg as KimibotConfig),
    setAccountEnabled: ({ cfg, accountId, enabled }) =>
      setAccountEnabledInConfigSection({
        cfg: cfg as KimibotConfig,
        sectionKey: "zalouser",
        accountId,
        enabled,
        allowTopLevel: true,
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as KimibotConfig).channels?.zalouser?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    self: async ({ cfg, accountId, runtime }) => {
      ...
      const account = resolveZalouserAccountSync({ cfg: cfg as KimibotConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZalouserAccountSync({ cfg: cfg as KimibotConfig });
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      return monitorZalouserProvider({
        account,
        config: ctx.cfg as KimibotConfig,
        runtime: ctx.runtime,
