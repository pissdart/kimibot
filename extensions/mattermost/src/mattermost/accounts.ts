...
export function listMattermostAccountIds(cfg: KimibotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: KimibotConfig): string {
...
export function listEnabledMattermostAccounts(cfg: KimibotConfig): ResolvedMattermostAccount[] {
