...
`kimi.invoke --each`) without adding new Kimibot code per workflow.
...
This extension depends on Kimibot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Kimibot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.kimibot/extensions` as a standalone plugin directory.
