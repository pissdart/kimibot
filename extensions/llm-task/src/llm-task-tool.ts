...
// NOTE: This extension is intended to be bundled with Kimibot.
// When running from source (tests/dev), Kimibot internals live under src/.
...
export function createLlmTaskTool(api: KimibotPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via kimi.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kimibot-llm-task-"));
