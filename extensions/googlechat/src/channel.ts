...
import { GoogleChatConfigSchema } from "kimibot/plugin-sdk";
...
export const googlechatDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveGoogleChatAccount({ cfg: cfg as KimibotConfig, accountId }).config.dm?.allowFrom ??
        []
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveGoogleChatAccount({ cfg: cfg as KimibotConfig });
  ...
  config: {
    listAccountIds: (cfg) => listGoogleChatAccountIds(cfg as KimibotConfig),
    resolveAccount: (cfg, accountId) =>
      resolveGoogleChatAccount({ cfg: cfg as KimibotConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultGoogleChatAccountId(cfg as KimibotConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as KimibotConfig).channels?.["googlechat"]?.accounts?.[resolvedAccountId],
      );
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const next =
        accountId !== DEFAULT_ACCOUNT_ID
          ? migrateBaseNameToDefaultAccount({
              cfg: namedConfig as KimibotConfig,
              channelKey: "googlechat",
            })
          : namedConfig;
